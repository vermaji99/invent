...
async function runPeriodReport(period) {
  try {
    console.log(`[Scheduler] Starting ${period} report generation at ${new Date().toLocaleString()}`);
    const { start, end } = periodStartEnd(period);
    console.log(`[Scheduler] Report period: ${start.toLocaleString()} to ${end.toLocaleString()}`);

    const invoices = await Invoice.find({ createdAt: { $gte: start, $lt: end } });
    console.log(`[Scheduler] Found ${invoices.length} invoices for ${period} report`);
    ...
    console.log(`[Scheduler] Sending ${period} report email to ${process.env.ADMIN_EMAIL}`);
    await sendEmail({ subject: `${period === 'weekly' ? 'Weekly' : 'Monthly'} Stock Report`, html, to: [process.env.ADMIN_EMAIL] });
    console.log(`[Scheduler] ${period} report sent successfully`);
  } catch (error) {
    console.error(`[Scheduler] Error generating/sending ${period} report:`, error);
  }
}
...
function initScheduler() {
  ...
  cron.schedule('10 10 * * 1', () => runPeriodReport('weekly'));
  cron.schedule('10 10 1 * *', () => runPeriodReport('monthly'));
